...
uint64_t get_entry_point(parser_t *pe)
{
  assert(pe);
  nt_header_32_t *nthdr = &pe->pe_header.nt;
  assert(nthdr->OptionalMagic == NT_OPTIONAL_64_MAGIC);

  printf("Image base %" PRIu64 ".\nAddressOfEntryPoint %u\n",
	 nthdr->OptionalHeader64.ImageBase,
	 nthdr->OptionalHeader64.AddressOfEntryPoint);
  
  return nthdr->OptionalHeader64.AddressOfEntryPoint + nthdr->OptionalHeader64.ImageBase;
}
